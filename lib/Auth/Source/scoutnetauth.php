...
class sspmod_scoutnetmodule_Auth_Source_scoutnetauth extends sspmod_core_Auth_UserPassBase
{
    ...
    protected function login(string $username, string $password): array
    {
        ...
        if (empty($authResultObj->member->member_no)) {
            ...
            if (empty($authResultObj->err)) {
                SimpleSAML_Logger::warning('ScoutnetAuth: Inget medlemsnummer för ' . var_export($username, TRUE) . '.');
            } else {
    ...
    /**
     * Removes diacritics from characters by replacing them with their ASCII equivalents.
     * Treats $str as UTF-8.
    ...
    private static function removeDiacritics(string $str): string
    {
        ...
        $result = preg_replace($replacements, array_keys($replacements), $str);
        if ($result === null) {
            throw new RuntimeException('Regexp replacement failed');
        }
